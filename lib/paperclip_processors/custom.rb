...
module Paperclip
  class Custom < Processor
    ...
    def make
      ...
      if watermark_path
        command = "composite"
        @watermark = Geometry.from_file watermark_path

        if @current_geometry.width < @target_geometry.width || @current_geometry.height < @target_geometry.height
          width_diff = @target_geometry.width - @current_geometry.width
          height_diff = @target_geometry.height - @current_geometry.height

          if height_diff > width_diff
            # horizontal
            proportion = (@current_geometry.height + width_diff) / @target_geometry.height
          else
            # vertical
            proportion = (@current_geometry.width + height_diff) / @target_geometry.width
          end
        else
          proportion = 1
        end

        params = %W[-gravity #{@position} \( #{watermark_path} -resize #{(proportion * 100)}% \) #{tofile(dst)}]
        params << tofile(dst)
        begin
          success = Paperclip.run(command, params.flatten.compact.collect{|e| "'#{e}'"}.join(" "))
        rescue Paperclip::Errors::CommandNotFoundError
